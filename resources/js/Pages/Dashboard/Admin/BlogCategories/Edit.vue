...
const props = defineProps({ blogCategory: { type: Object, required: true } });
const page = usePage();
const languages = computed(() => page.props.languages ?? []);

const form = useForm({
    name: { ...props.blogCategory.name_translations },
});
...
function submit() {
    form.transform((data) => ({ ...data, _method: 'put' })).post(`/admin/blog-categories/${props.blogCategory.id}`, {
        preserveScroll: true,
    });
}
</script>
...
<template>
    <div>
        ...
        <div class="mb-6"><Link href="/admin/blog-categories" class="text-sm font-medium text-sky-700">← Blog categories</Link><h1 class="mt-2 text-xl font-semibold text-slate-900">Edit blog category</h1></div>
