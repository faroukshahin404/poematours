...
const form = useForm({
    ...
    name: { ...props.destination.name_translations },
...
function submit() {
    form
        ...
        .post(`/admin/destinations/${props.destination.id}`, {
            preserveScroll: true,
            forceFormData: true,
        });
}
...
<template>
    <div>
        ...
        <div class="mb-6">
            ...
            <h1 class="mt-2 text-xl font-semibold text-slate-900">Edit destination</h1>
        ...
        >
            <div v-if="destination.image_url" class="flex items-start gap-4">
                <div class="size-24 shrink-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-100">
                    <img :src="destination.image_url" alt="" class="size-full object-cover" />
                </div>
