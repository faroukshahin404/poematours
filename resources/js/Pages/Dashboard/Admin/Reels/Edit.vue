...
function submit() {
    form.transform((data) => ({ ...data, _method: 'put' })).post(`/admin/reels/${props.reel.id}`, {
        preserveScroll: true,
    });
}
</script>
...
<template>
    <div>
        <Head title="Edit reel" />
        <div class="mb-6">
            <Link href="/admin/reels" class="text-sm font-medium text-sky-700">← Reels</Link>
            <h1 class="mt-2 text-xl font-semibold text-slate-900">Edit reel</h1>
        </div>
