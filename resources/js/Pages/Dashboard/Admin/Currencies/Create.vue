...
const isFirstCurrency = computed(() => props.currencyCount === 0);
...
function submit() {
    if (isFirstCurrency.value) {
        form.is_default = true;
    }
    form.post('/admin/currencies', {
        preserveScroll: true,
    });
}
</script>
...
<template>
    <div>
        ...
        <div class="mb-6">
            <Link href="/admin/currencies" class="text-sm font-medium text-sky-700 hover:text-sky-900">
                ← Currencies
            </Link>
            <h1 class="mt-2 text-xl font-semibold text-slate-900">Add currency</h1>
        ...
        >
            ...
            <div class="rounded-lg border border-dashed border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-600">
                ...
                <span class="mt-1 block text-xs text-slate-500">Final slug is saved on the server and stays unique.</span>
            </div>

            <div v-if="isFirstCurrency" class="rounded-lg border border-sky-100 bg-sky-50 px-3 py-2 text-sm text-sky-900">
                The first currency is always set as the default site currency.
            </div>
            ...
            <div v-else class="flex items-start gap-3">
                ...
                />
                <div>
                    <label for="is_default" class="text-sm font-medium text-slate-700">Default currency</label>
                    <p class="mt-0.5 text-xs text-slate-500">
                        Only one currency can be default. Checking this will unset the current default.
                    </p>
