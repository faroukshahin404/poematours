...
function submit() {
    form.post('/admin/package-inclusions', { preserveScroll: true });
}
</script>

<template>
    <div>
        <Head title="Add package inclusion" />
        <div class="mb-6">
            <Link href="/admin/package-inclusions" class="text-sm font-medium text-sky-700">← Package inclusions</Link>
            <h1 class="mt-2 text-xl font-semibold text-slate-900">Add package inclusion</h1>
        </div>
        <form class="max-w-3xl space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
            ...
            <div>
                ...
                />
                <p class="mt-1 text-xs text-slate-500">Stored as plain text for display on the site.</p>
            </div>
