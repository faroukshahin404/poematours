...
const form = useForm({
    ...
    country_id: props.contact.country_id,
    status: props.contact.status,
    source: props.contact.source ?? 'manual',
    service_ids: props.contact.service_ids ?? [],
    notes: props.contact.notes ?? '',
});

function submit() {
    form.put(`/admin/crm-contacts/${props.contact.id}`, {
        preserveScroll: true,
    });
}
</script>
...
<template>
    <div>
        ...
        <div class="mb-6">
            ...
            <h1 class="mt-2 text-xl font-semibold text-slate-900">Edit contact</h1>
        </div>
        ...
        <form class="max-w-2xl space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
            <div class="grid gap-5 md:grid-cols-2">
                ...
                <div>
                    ...
                    >
                        <option v-for="sourceOption in sourceOptions" :key="sourceOption.value" :value="sourceOption.value">
                            {{ sourceOption.label }}
                        </option>
                    </select>
