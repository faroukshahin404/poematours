...
const form = useForm({
    title: { ...(props.reservationQuestion.title ?? {}) },
    description: { ...(props.reservationQuestion.description ?? {}) },
    type: props.reservationQuestion.type,
    is_package_reservation: Boolean(props.reservationQuestion.is_package_reservation),
    is_reservation_page: Boolean(props.reservationQuestion.is_reservation_page),
    options: (props.reservationQuestion.options ?? []).map((option) => ({
        label: { ...(option.label ?? {}) },
        added_price: option.added_price ?? 0,
    })),
});
...
function submit() {
    form
        ...
        .post(`/admin/reservation-questions/${props.reservationQuestion.id}`, {
            preserveScroll: true,
        });
}
</script>
...
<template>
    <div>
        <Head title="Edit reservation question" />

        <div class="mb-6">
            <Link href="/admin/reservation-questions" class="text-sm font-medium text-sky-700 hover:text-sky-900">
                ← Reservation questions
            </Link>
            <h1 class="mt-2 text-xl font-semibold text-slate-900">Edit reservation question</h1>
        </div>
