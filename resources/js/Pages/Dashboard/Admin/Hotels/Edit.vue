...
const form = useForm({
    slug: props.hotel.slug,
    destination_id: props.hotel.destination_id,
    stars: props.hotel.stars,
    name: { ...props.hotel.name_translations },
    description: { ...props.hotel.description_translations },
    images: [],
    remove_media_ids: [],
});
...
function submit() {
    form.transform((data) => ({ ...data, _method: 'put' })).post(`/admin/hotels/${props.hotel.id}`, {
        preserveScroll: true,
        forceFormData: true,
    });
}
...
<template>
    <div>
        ...
        <div class="mb-6">
            ...
            <h1 class="mt-2 text-xl font-semibold text-slate-900">Edit hotel</h1>
        </div>
        ...
        <form class="max-w-3xl space-y-6 rounded-xl border border-slate-200 bg-white p-6 shadow-sm" @submit.prevent="submit">
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
                <div>
                    ...
                    <select v-model="form.destination_id" class="block w-full rounded-lg border border-slate-200 px-3 py-2 text-sm">
                ...
                <div>
                    ...
                    <input v-model="form.stars" type="number" min="1" max="7" class="block w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
                </div>
                <div>
            ...
            <div class="space-y-4">
                ...
                <div v-for="lang in languages" :key="`description-${lang.slug}`">
                    <label class="mb-1 block text-sm font-medium text-slate-700">{{ lang.name }} ({{ lang.slug }})</label>
                    <textarea v-model="form.description[lang.slug]" rows="3" class="block w-full rounded-lg border border-slate-200 px-3 py-2 text-sm" />
                </div>
            </div>
            ...
            <div class="space-y-3">
                ...
                <div class="grid grid-cols-2 gap-3 md:grid-cols-4">
                    <label v-for="media in hotel.images" :key="media.id" class="block rounded-lg border p-2" :class="form.remove_media_ids.includes(media.id) ? 'border-red-300 bg-red-50' : 'border-slate-200'">
